...
struct GridMapping {
  ...
  inline float resolution() const { return _g2w.linear()(0,0); }
  inline const Vector2f origin() const { return _g2w.translation(); }
  ...
  inline Vector2f world2grid(const Vector2f& src) const { return _w2g*src; }
  inline Vector2f grid2world(const Vector2f& src) const { return _g2w*src; }
  inline Eigen::Matrix2f world2gridDerivative() const { return _w2g.linear(); }
};
...
struct GridMap : public Grid_<uint8_t>, public GridMapping {
  ...
  inline const Vector2f center() const {
    return grid2world(Vector2f(this->cols/2, this->rows/2));
  }

  bool scanRay(Vector2f& hit, const Vector2f& origin, const Vector2f& direction,
               const float max_range) const;
  float scanRay(const Vector2f& origin, const Vector2f& direction,
                const float max_range) const;
};
