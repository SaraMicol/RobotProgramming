...
// Callback per goal RViz
void goalCallback(const geometry_msgs::PoseStamped::ConstPtr& goal, 
                  RobotConfig* robot, 
                  RobotGoal* robot_goal)
{
    robot_goal->has_goal = true;
    robot_goal->goal_x = goal->pose.position.x;
    robot_goal->goal_y = goal->pose.position.y;
    
    ROS_INFO("Robot %s: Nuovo goal ricevuto (%.2f, %.2f)", 
             robot->id.c_str(), robot_goal->goal_x, robot_goal->goal_y);
}
...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < robots.size(); ++i) {
        ...
        // Subscriber per goal individuali
        goal_subs.push_back(nh.subscribe<geometry_msgs::PoseStamped>(
            "/" + robots[i].id + "/move_base_simple/goal", 10,
            boost::bind(&goalCallback, _1, &robots[i], &robot_goals[i])
        ));

        ROS_INFO("Robot %s: Pronto. Topic goal: /%s/move_base_simple/goal",
                 robots[i].id.c_str(), robots[i].id.c_str());
    }
    ...
    ros::Subscriber global_goal_sub = nh.subscribe<geometry_msgs::PoseStamped>(
        ...
        [&](const geometry_msgs::PoseStamped::ConstPtr& goal_msg){
            for (size_t i = 0; i < robots.size(); ++i) {
                ...
                ROS_INFO("Robot %s: Global goal ricevuto (%.2f, %.2f)",
                        robots[i].id.c_str(),
                        robot_goals[i].goal_x,
                        robot_goals[i].goal_y);
            }
        }
    );
